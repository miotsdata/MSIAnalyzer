...
| `summary_report.html` | `out_dir` | run | per-sample counts, feature-overlap UpSet plot, MS2 association (overall + per sample) and purity (overall histogram + per-sample distribution), a recheck of the unassociated MS2 against each sample's pre-filter MS1 peaks, and a breakdown of *why* purity is unscored (faint precursor / off-pixel flyback / no precursor m/z) |
